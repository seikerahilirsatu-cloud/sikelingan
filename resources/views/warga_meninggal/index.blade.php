...
<div class="{{ (isset($is_mobile) && $is_mobile) ? 'max-w-3xl mx-auto' : 'container-fluid px-0' }}">
    <div class="mb-4">
        ...
        <div class="mb-4 mt-2">
            <form action="{{ route('warga_meninggal.index') }}" method="get" class="flex flex-col sm:flex-row items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('warga_meninggal.index') }}" class="h-10 inline-flex items-center px-4 border border-red-300 bg-red-50 text-red-700 rounded-lg text-sm hover:bg-red-100">Reset</a>
                    <a href="{{ route('warga_meninggal.create') }}" data-modal="true" class="h-10 inline-flex items-center px-4 bg-indigo-600 text-white rounded-lg" role="button" aria-label="Tambah pencatatan">Tambah</a>
                </div>
            </form>
        </div>
    </div>
    ...
    @if($items->count())
                        ...
                        <div class="flex items-start justify-between">
                            ...
                            <div class="text-right">
                                ...
                                <div class="mt-2 flex items-center gap-2">
                                    @if(Route::has('warga_meninggal.edit'))
                                        <a href="{{ route('warga_meninggal.edit', $it) }}" data-modal="true" class="inline-flex items-center px-3 py-1 bg-yellow-500 text-white rounded-md text-sm hover:bg-yellow-600">Edit</a>
                                    @endif
                                    <form action="{{ route('warga_meninggal.destroy', $it) }}" method="POST" onsubmit="return confirm('Hapus pencatatan warga meninggal untuk @db($it->warga?->nama_lgkp ?? $it->warga?->nama)?')">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="table-responsive">
            <table class="table table-modern table-hover col-date-3 col-address-6 col-actions-last center-data-5">
                    <thead>
                        <tr>
                            <th>NIK</th>
                            <th>Nama</th>
                    ...
                    <tbody>
                        @foreach($items as $it)
                            <tr>
                                ...
                                <td>{{ optional($it->tanggal_meninggal)->format('Y-m-d') ?? '-' }}</td>
                                <td>@db($it->waktu_meninggal ?? '-')</td>
                                <td>@db($it->lingkungan ?? ($it->warga?->lingkungan ?? '-'))</td>
                                <td>@db($it->alamat ?? ($it->warga?->alamat ?? '-'))</td>
                                <td>@db($it->creator?->name ?? '-')</td>
                                <td>
                                    @if(Route::has('warga_meninggal.show'))
                                        <a href="{{ route('warga_meninggal.show', $it) }}" data-modal="true" class="btn btn-sm btn-primary">Detail</a>
                                    @endif
                                    @if(Route::has('warga_meninggal.edit'))
                                        <a href="{{ route('warga_meninggal.edit', $it) }}" data-modal="true" class="btn btn-sm btn-warning">Edit</a>
                                    @endif
                                    <form action="{{ route('warga_meninggal.destroy', $it) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus pencatatan warga meninggal untuk @db($it->warga?->nama_lgkp ?? $it->warga?->nama)?')">
                                        @csrf
                                        @method('DELETE')
    ...
    @else
        ...
        @elseif((isset($from) && $from) || (isset($to) && $to))
            <div class="text-sm text-gray-600">Tidak ada catatan meninggal dari tanggal <span class="font-medium">{{ $from ?? '-' }}</span> s.d <span class="font-medium">{{ $to ?? '-' }}</span>.</div>
        @else
            <div class="text-sm text-gray-600">Belum ada pencatatan warga meninggal.</div>
        @endif
    @endif
</div>
