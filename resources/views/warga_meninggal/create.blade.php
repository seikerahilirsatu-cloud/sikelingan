<x-app-layout>
    <x-slot name="header">
        ...
        <h2 class="font-semibold text-sm sm:text-xl text-gray-800 leading-tight page-title">Tambah Pencatatan Warga Meninggal</h2>
        @endif
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto p-4">
            <div class="bg-white shadow sm:rounded-lg p-6">
                ...
                <form method="GET" action="{{ route('warga_meninggal.create') }}" class="mb-4">
                ...
                @if(isset($selectedWarga) && $selectedWarga)
                    <form method="POST" action="{{ route('warga_meninggal.store') }}">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="tanggal_meninggal" required class="block w-full rounded-md border-gray-300 px-3 py-2" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Waktu Meninggal</label>
                                <input type="time" name="waktu_meninggal" class="block w-full rounded-md border-gray-300 px-3 py-2" />
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="sebab_meninggal" rows="2" class="block w-full rounded-md border-gray-300 px-3 py-2"></textarea>
                        </div>

                        <div class="mt-4">
                            <label class="block text-sm font-medium text-gray-700">Tanggal Dikebumikan</label>
                            <input type="date" name="tanggal_dikebumikan" class="block w-full rounded-md border-gray-300 px-3 py-2" />
                        </div>
                        ...
                        <div class="mt-4 flex justify-between">
                            <a href="{{ route('warga_meninggal.create', ['q' => $q ?? '']) }}" class="px-3 py-2 border rounded text-sm text-gray-700">Kembali ke daftar</a>
                    ...
                    @if(isset($wargas) && $wargas->count())
                        <div class="mb-4">
                            ...
                            <div id="wargaCards" class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                                @foreach($wargas as $w)
                                    <a href="{{ route('warga_meninggal.create', ['warga_id' => $w->id, 'q' => $q ?? '']) }}" class="block select-warga-card bg-white p-3 rounded-lg border border-transparent hover:shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-400">
