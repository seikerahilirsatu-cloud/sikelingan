...
<div class="max-w-3xl mx-auto">
    <div class="mb-3">
        <a href="{{ route('import.rumah_ibadah.form', absolute: false) }}" class="text-sm text-gray-600">Kembali</a>
        <h1 class="text-2xl font-semibold">Preview Import</h1>
    </div>
    <form method="post" action="{{ route('import.rumah_ibadah.commit', absolute: false) }}" class="bg-white p-4 rounded shadow">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-3">
            @foreach($items as $i)
                <div class="border rounded p-3">
                    ...
                    <div class="text-sm font-semibold">{{ $i['data']['nama'] ?? '-' }}</div>
