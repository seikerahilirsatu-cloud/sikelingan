...
@section('page_title','Daftar Warga Pindah Keluar')
@section('content')
<div class="{{ (isset($is_mobile) && $is_mobile) ? 'max-w-3xl mx-auto' : 'container-fluid px-0' }}">
    <div class="mb-4">

        <div class="mb-4 mt-2">
            <form action="{{ route('pindah_keluar.index') }}" method="get" class="flex flex-col sm:flex-row items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('pindah_keluar.index') }}" class="h-10 inline-flex items-center px-4 border border-red-300 bg-red-50 text-red-700 rounded-lg text-sm hover:bg-red-100">Reset</a>
                    <a href="{{ route('pindah_keluar.create') }}" data-modal="true" class="h-10 inline-flex items-center px-4 bg-indigo-600 text-white rounded-lg" role="button" aria-label="Tambah pencatatan">Tambah</a>
                </div>
            </form>
        </div>
    </div>
                        ...
                        <div class="flex items-start justify-between">
                            ...
                            <div class="text-right">
                                ...
                                <div class="mt-2 flex items-center gap-2">
                                    @if(Route::has('pindah_keluar.edit'))
                                        <a href="{{ route('pindah_keluar.edit', $it) }}" data-modal="true" class="inline-flex items-center px-3 py-1 bg-yellow-500 text-white rounded-md text-sm hover:bg-yellow-600">Edit</a>
                                    @endif
                                    <form action="{{ route('pindah_keluar.destroy', $it) }}" method="POST" onsubmit="return confirm('Hapus pencatatan pindah keluar untuk @db($it->warga?->nama_lgkp ?? $it->warga?->nama)?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    <tbody>
                        @foreach($items as $it)
                            <tr>
                                ...
                                <td>{{ optional($it->tanggal_pindah)->format('Y-m-d') ?? '-' }}</td>
                                <td>@db($it->lingkungan ?? ($it->warga?->lingkungan ?? '-'))</td>
                                <td>@db($it->alamat ?? ($it->warga?->alamat ?? '-'))</td>
                                <td>@db($it->creator?->name ?? '-')</td>
                                <td>
                                    @if(Route::has('pindah_keluar.show'))
                                        <a href="{{ route('pindah_keluar.show', $it) }}" data-modal="true" class="btn btn-sm btn-primary">Detail</a>
                                    @endif
                                    @if(Route::has('pindah_keluar.edit'))
                                        <a href="{{ route('pindah_keluar.edit', $it) }}" data-modal="true" class="btn btn-sm btn-warning">Edit</a>
                                    @endif
                                    <form action="{{ route('pindah_keluar.destroy', $it) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus pencatatan pindah keluar untuk @db($it->warga?->nama_lgkp ?? $it->warga?->nama)?')">
                                        @csrf
                                        @method('DELETE')
    ...
    @else
        ...
        @elseif((isset($from) && $from) || (isset($to) && $to))
            ...
            {{-- Set a sessionStorage marker so a subsequent refresh will clear filters only on reload --}}
            <script>
                (function(){
                    try {
                        var key = 'pindah_no_results_marker';
                        var urlFrom = "{{ $from ?? '' }}";
                        var urlTo = "{{ $to ?? '' }}";

                        var existing = sessionStorage.getItem(key);
                        if (!existing) {
                            // first time viewing the no-result page: store marker
                            sessionStorage.setItem(key, JSON.stringify({from: urlFrom, to: urlTo}));
                        } else {
                            // if marker exists and matches current filters, this is likely a reload -> clear filters
                            try {
                                var obj = JSON.parse(existing);
                                if (obj && obj.from === urlFrom && obj.to === urlTo) {
                                    sessionStorage.removeItem(key);
                                    // redirect to path without query string to clear filters
        ...
        @else
            <div class="text-sm text-gray-600">Belum ada pencatatan pindah keluar.</div>
        @endif
    @endif
</div>
