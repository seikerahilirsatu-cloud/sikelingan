...
<div class="max-w-3xl mx-auto">
    <div class="mb-4">
        <h1 class="text-2xl font-semibold">Daftar Warga Pindah</h1>
        ...
        <div class="mb-4 mt-2">
            <form action="{{ route('pindah_keluar.index') }}" method="get" class="flex flex-col sm:flex-row items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('pindah_keluar.create') }}" class="h-10 inline-flex items-center px-4 bg-indigo-600 text-white rounded-lg" role="button" aria-label="Tambah pencatatan">Tambah</a>
                </div>
            </form>
        </div>
    </div>
                ...
                <article class="bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <div class="text-xs text-gray-600 mt-1">Jenis: <span class="font-medium">{{ $it->jenis_pindah }}</span></div>
                            <div class="text-sm text-gray-600 mt-2">Tujuan: {{ $it->tujuan }}</div>
                        ...
                        <div class="text-right">
                            ...
                            <div class="mt-2 flex items-center gap-2">
                                @if(Route::has('pindah_keluar.edit'))
                                    <a href="{{ route('pindah_keluar.edit', $it) }}" class="inline-flex items-center px-3 py-1 bg-yellow-500 text-white rounded-md text-sm hover:bg-yellow-600">Edit</a>
                                @endif

                                <form action="{{ route('pindah_keluar.destroy', $it) }}" method="POST" onsubmit="return confirm('Hapus pencatatan pindah keluar untuk {{ $it->warga?->nama_lgkp ?? $it->warga?->nama }}?')">
                                    @csrf
                                    @method('DELETE')
