...
<div class="space-y-3">
    <div class="bg-white p-3 rounded">
        ...
        <div class="text-xs text-gray-600">{{ $item->alamat }}</div>
        <div class="text-xs text-gray-600">Lingkungan: {{ $item->lingkungan ?? '-' }}</div>
        <div class="text-xs text-gray-600">Status: {{ $item->status_operasional }}</div>
        @if($item->pemilik_nik)
        <div class="text-xs text-gray-600">Pemilik NIK: {{ $item->pemilik_nik }}</div>
        @endif
        @if($item->npwp_pemilik)
        <div class="text-xs text-gray-600">NPWP Pemilik: {{ $item->npwp_pemilik }}</div>
        @endif
        @if($item->no_nib)
        <div class="text-xs text-gray-600">No NIB: {{ $item->no_nib }}</div>
        @endif
