...
<div class="max-w-2xl mx-auto">
    <div class="mb-4">
        ...
        <h1 class="text-2xl font-semibold">Tambah Data Keluarga</h1>
    </div>
    <form method="post" action="{{ route('data_keluarga.store') }}" class="bg-white shadow-md rounded-lg p-6 space-y-3">
        @csrf
        <div>
            <label class="block text-sm">No. KK</label>
            <input name="no_kk" value="{{ old('no_kk') }}" class="w-full p-2 border rounded" />
            @error('no_kk')<div class="text-xs text-red-600">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block text-sm">Nama Kepala</label>
            <input name="nama_kep" value="{{ old('nama_kep') }}" class="w-full p-2 border rounded" />
        ...
        <div>
            ...
            @php $user = auth()->user(); $isKL = ($user->role ?? null) === 'kepala_lingkungan'; @endphp
            @php $opts = config('app_local.lingkungan_opts'); @endphp
            <select name="lingkungan" class="w-full p-2 border rounded" {{ $isKL ? 'disabled' : '' }}>
                <option value="">-- Pilih Lingkungan --</option>
                @foreach($opts as $ling)
                    @php $selected = old('lingkungan', $defaultLingkungan ?? null) == $ling ? 'selected' : ''; @endphp
                    <option value="{{ $ling }}" {{ $selected }}>{{ $ling }}</option>
                @endforeach
            </select>
            @if($isKL)
                <input type="hidden" name="lingkungan" value="{{ $defaultLingkungan }}" />
            @endif
        ...
        <div>
            ...
            <select name="status_keluarga" class="w-full p-2 border rounded">
                <option value="1">Warga Domisili</option>
                <option value="2">Warga Luar Domisili</option>
                <option value="3">Warga Domisili Baru</option>
            </select>
        </div>
        <div class="pt-2">
