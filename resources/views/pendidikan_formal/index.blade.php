...
<div class="max-w-3xl mx-auto">
  ...
  <div class="flex items-center justify-between mb-4">
    <form id="searchFormFormal" class="flex items-center gap-2" action="{{ route('pendidikan_formal.index', absolute: false) }}" method="get">
      <input id="searchInputFormal" name="q" value="{{ $q ?? '' }}" placeholder="Cari nama, jenjang, alamat" class="w-72 p-2 border rounded-lg" />
    </form>
    <script>
    (function(){
    ...
    <a href="{{ route('pendidikan_formal.create', absolute: false) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg" role="button">Tambah</a>
  </div>
