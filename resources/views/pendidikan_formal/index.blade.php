...
@section('page_title','Sarana Pendidikan Formal')
@section('content')
<div class="{{ (isset($is_mobile) && $is_mobile) ? 'max-w-3xl mx-auto' : 'container-fluid px-0' }}">
    <div class="mb-4"></div>

  <div class="flex items-center gap-3 mb-4">
    <form id="searchFormFormal" class="flex-1" action="{{ route('pendidikan_formal.index') }}" method="get">
      <div class="relative">
    ...
    <a href="{{ route('pendidikan_formal.create') }}" data-modal="true" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-300/50" role="button">Tambah</a>
  </div>
          ...
          <article class="bg-white p-4 rounded-lg shadow-sm">
            <div class="flex items-start justify-between">
              <div>
                ...
                @else
                  <img src="{{ asset('images/placeholder-ibadah.svg') }}" alt="{{ $p->nama_sekolah }}" class="mb-3 w_full rounded-md" style="max-height: 90px; max-width: 50%; object-fit: cover;" />
                @endif
                ...
                <div class="text-xs text_gray-600 mt-1">Lingkungan: <span class="font-medium">@db($p->lingkungan ?? '-') </span></div>
                <div class="text-xs text-gray-600 mt-1">Status: @db($p->stts_sekolah)</div>
              </div>
              <div class="text-right">
                <a href="{{ route('pendidikan_formal.edit', $p) }}" data-modal="true" class="inline-flex items-center px-3 py-1 bg-yellow-500 text-white rounded-md text-sm">Edit</a>
                <form action="{{ route('pendidikan_formal.destroy', $p) }}" method="post" class="mt-2">
                  @csrf @method('DELETE')
                  <button class="inline-flex items_center px-3 py-1 bg-red-600 text-white rounded-md text-sm" onclick="return confirm('Hapus data ini?')">Hapus</button>
                </form>
              </div>
            </div>
          </article>
    ...
    <div class="table-responsive">
        <table class="table table-modern table-hover col-address-3 col-status-5 col-actions-last center-data-4">
          ...
          <tbody>
            ...
            <tr>
              <td>@db($p->jenjang)</td>
              <td>@db($p->nama_sekolah)</td>
              <td>@db($p->alamat)</td>
              <td>@db($p->lingkungan ?? '-') </td>
              <td>@db($p->stts_sekolah)</td>
              <td>
                <a href="{{ route('pendidikan_formal.show', $p) }}" data-modal="true" class="btn btn-sm btn-primary">Detail</a>
                <a href="{{ route('pendidikan_formal.edit', $p) }}" data-modal="true" class="btn btn-sm btn-warning text-white">Edit</a>
                <form action="{{ route('pendidikan_formal.destroy', $p) }}" method="post" class="d-inline">
                  @csrf @method('DELETE')
