...
<div class="max-w-3xl mx-auto">
  <div class="mb-4">
    ...
    <a href="{{ auth()->check() ? route('pendidikan_formal.index', absolute: false) : (url()->previous() ?? url('/')) }}" class="inline-flex items-center text-sm text-gray-600 hover:text-gray-800 mb-2">Kembali</a>
    @endif
    <h1 class="text-2xl font-semibold page-title">Detail data @db($item->nama_sekolah)</h1>
  </div>
  ...
  <div class="bg-white rounded-2xl shadow p-4">
    ...
    <div class="mt-4 flex gap-2">
      @if(isset($is_mobile) && $is_mobile)
      <a href="{{ auth()->check() ? route('pendidikan_formal.index', absolute: false) : (url()->previous() ?? url('/')) }}" class="px-3 py-2 border rounded">Kembali</a>
      @endif
      @if(auth()->check())
