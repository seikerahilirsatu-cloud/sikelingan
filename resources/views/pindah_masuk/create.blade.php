<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-sm sm:text-xl text-gray-800 leading-tight page-title">Tambah Pencatatan Pindah Masuk</h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto p-4">
            <div class="bg-white shadow sm:rounded-lg p-6">
                ...
                <form method="GET" action="{{ route('pindah_masuk.create') }}" class="mb-4">
                ...
                @if(isset($selectedWarga) && $selectedWarga)
                    <form method="POST" action="{{ route('pindah_masuk.store') }}">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="tanggal_masuk" required class="block w-full rounded-md border-gray-300 px-3 py-2" />
                            </div>
                            <div>
                        ...
                        <div class="mt-4">
                            ...
                            <input type="text" name="asal" class="block w-full rounded-md border-gray-300 px-3 py-2" />
                        </div>
                        ...
                        <div class="mt-4 flex justify-between">
                            <a href="{{ route('pindah_masuk.create', ['q' => $q ?? '']) }}" class="px-3 py-2 border rounded text-sm text-gray-700">Kembali ke daftar</a>
                    ...
                    @if(isset($wargas) && $wargas->count())
                        <div class="mb-4">
                            ...
                            <div id="wargaCards" class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                                @foreach($wargas as $w)
                                    <a href="{{ route('pindah_masuk.create', ['warga_id' => $w->id, 'q' => $q ?? '']) }}" class="block select-warga-card bg-white p-3 rounded-lg border border-transparent hover:shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-400">
